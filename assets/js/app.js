...
gifAddition = (arr) =>{
  for (var i = 0; i < arr.length; i++) {
    ...
    let title = $("<span>").addClass("card-title activator grey-text text-darken-4").text(arr[i].title)
    let moreIcon = $("<i>").addClass("material-icons right").text("more_vert")
    let reveal = $("<div>").addClass("card-reveal")
    let close = $("<span>").addClass("card-title grey-text text-darken-4").text(arr[i].title)
    let closeIcon = $("<i>").addClass("material-icons right").text("close")
